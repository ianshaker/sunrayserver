...
function buildRouterPrompt(enabledIntents, chatContext = null) {
  ...
  if (hasLoading && !hasAppeal) {
    ...
    chatHint =
      "В чате есть и loading_deadline, и appeal_deadline — смотри маркеры «погрузк*/Дедлайн #… с датой на следующей строке/ДЕДЛАЙН ПОГРУЗКИ» vs «входящ*/ДЕДЛАЙН #… - дата» капсом в одну строку.";
  }
...
- Если есть контекст reply и в нём карточка погрузки — «⏰ Дедлайн #…» с датой на следующей строке, пинг «⏰ Дедлайн #… не закрыт» или старая «ДЕДЛАЙН ПОГРУЗКИ #…» — команды про перенос/обновление инфо → intent loading_deadline_manage (не appeal_deadline_*).
- Если есть контекст reply и в нём карточка «ДЕДЛАЙН #… - дата» капсом в одну строку (входящее обращение) — команды про перенос дедлайна, отказ, погрузку, добавление инфо → intent appeal_deadline_manage (даже если номер заявки только в reply, а не в тексте команды).
