...
function buildRouterPrompt(enabledIntents, chatContext = null) {
  ...
  if (hasLoading && !hasAppeal) {
    chatHint =
      ...
      "Команды «перенеси дедлайн», «обнови телефон», «добавь инфо» без уточнения «входящие» → loading_deadline_manage. " +
...
- Вопросы/просьбы ПОКАЗАТЬ дедлайны по погрузке / по замерам / в отделе погрузки («дай», «скинь», «покажи», «какие», «есть ли», «самый срочный») → loading_deadline_query, НЕ appeal_deadline_* и НЕ task_*.
- Вопросы/просьбы ПОКАЗАТЬ дедлайны по входящим/заявкам/обращениям («дай», «покажи», «какие», «есть ли», «список», «самый срочный») → appeal_deadline_query, НЕ appeal_deadline_manage и НЕ task_* и НЕ loading_deadline_*.
...
- loading_deadline_manage — действие над событием погрузки: перенести дедлайн, добавить инфо, отправить в отказ, или назначить замер (мастер + дата + время). Обычно reply на «ДЕДЛАЙН ПОГРУЗКИ» или команда в чате с правом loading_deadline.
