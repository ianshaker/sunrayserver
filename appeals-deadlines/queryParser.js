// ============================================================================
// Извлечение параметров запроса дедлайнов по входящим через Gemini.
// Модель отдаёт только «что спросили» — список заявок читает код.
// ============================================================================
...
function clarify(question) {
  return {
    status: "clarify",
    question: question || "Уточните: дедлайны по входящим обращениям или по задачам?",
  };
}
...
function unsupported(message) {
  return {
    ...
    message:
      message ||
      "Этот запрос не про просмотр дедлайнов. Для переноса/отказа/погрузки ответьте на карточку ДЕДЛАЙН с номером заявки.",
  };
}
 ...
 * @returns {Promise<
 *   | { status: "ok", mode: "by_date"|"urgent", date: string|null, limit: number, all: boolean, domainOk: boolean }
...
async function parseDeadlineQuery(text, { replyText } = {}) {
  if (!text || !text.trim()) {
    return clarify("Уточните: какие дедлайны по входящим показать и на какую дату?");
  }
  if (!hasCredentials()) {
    console.log("[appeals-deadlines/queryParser] AI недоступен (нет credentials)");
  ...
  console.log(`[appeals-deadlines/queryParser] запрос Gemini, длина=${text.trim().length}`);
  ...
  if (!raw) {
    console.log(
      `[appeals-deadlines/queryParser] пустой ответ Gemini: finish=${finishReason || "?"}`,
    );
    return {
      status: "error",
  ...
  if (!parsed) {
    console.log("[appeals-deadlines/queryParser] JSON не разобран");
  ...
  if (mode === "by_date") {
    ...
    if (!DATE_RE.test(date)) {
      return clarify("Не понял дату — скажите, на какой день показать дедлайны по входящим?");
    }
  }
