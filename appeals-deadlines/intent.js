    ...
    const errMsg =
      parsed.error === "ai_disabled"
        ...
        : parsed.error === "gemini_timeout"
          ? "AI не успел ответить. Попробуйте короче, например:\n<code>@SUNRAYY_bot добавить тел 89936014136, описание: дорого, дедлайн оставить на 1 июля</code>"
...
module.exports = {
  ...
  description:
    "Менеджер ДЕЙСТВУЕТ по конкретной входящей заявке: переносит дедлайн, " +
    "ставит отказ, отправляет в погрузку или добавляет инфо. " +
    "Обычно есть номер заявки (#NNNNN) или reply на карточку «ДЕДЛАЙН #…». " +
    "НЕ для вопросов «какие/дай/покажи дедлайны» — это appeal_deadline_query.",
  examples: [
